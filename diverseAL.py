    ...
    def run(self):
        ...
        while queried < config.max_queried:
            ...
            if len(result) >= self.num_select:
                ...
                # 使用不确定性采样策略
                print("开始不确定性采样...")
                U_best = UncertaintySamplingEntropy(self.num_select, {img: result[img] for img in image_paths})
                ...
                # 使用多样性采样策略
                print("开始多样性采样...")
                diversity_sampler = InformativeClusterDiverseSampler(selected_features, [0] * len(U_best), seed=42, selected_results=selected_results )
                diversity_selected_indices = diversity_sampler.select_batch_(self.model, [], self.num_select2)
                diversity_selected_images = [U_best[i] for i in diversity_selected_indices]
                diversity_features = np.array([self.model.extract_features(img) for img in diversity_selected_images])
                print(f"多样性采样完成，共选择了 {len(diversity_selected_images)} 张图像。")


                # 复制选择的图像文件
                for image_path in diversity_selected_images:
                ...
                # 更新 train.txt 文件
                with open('C://system//system//1//data//new//train.txt', "a") as f:
                    for file_name in diversity_selected_images:
                        # 替换路径中的反斜杠为正斜杠
                ...
                remaining_files = [
                    ...
                    for root, _, files in os.walk(
                        'C://system//system//1//data//new//unlabeled')
...
if __name__ == '__main__':
    txt_file_path = 'C://system//system//1//data//new//unlabeledpc.txt'
    folder_path_t = 'C://system//system//1//data//new//train'
    txt_folder_path = 'C://system//system//1//data//new//txt_all'  # 源txt文件夹路径
    txt_new_folder_path = 'C://system//system//1//data//new//train'  # 目标txt文件夹路径
