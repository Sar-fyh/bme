...
import sys
sys.path.append(r'C://system//system//1//models//utils')
...
class EnhancedConv(nn.Module):
    # 卷积 + SE + CBAM
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, use_se=False, use_cbam=False):
        super(EnhancedConv, self).__init__()
        self.conv = Conv(c1, c2, k, s, p, g, act)
        self.se = SE(c2) if use_se else nn.Identity()
        self.cbam = CBAM(c2) if use_cbam else nn.Identity()
